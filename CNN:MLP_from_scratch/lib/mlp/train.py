...
def train_net(data, model, loss_func, optimizer, batch_size, max_epochs,
    ...
    for epoch in range(max_epochs):
        ...
        for iter in tqdm(range(iter_start, iter_end)):
            ...
            #############################################################################
            res = model.forward(data_batch)
            loss = loss_func.forward(res, labels_batch)
            loss_grid = loss_func.backward()
            if regularization == 'l1':
                model.backward(loss_grid, regularization='l1', reg_lambda=reg_lambda)
            elif regularization == 'l2':
                model.backward(loss_grid, regularization='l2', reg_lambda=reg_lambda)
            else:
                model.backward(loss_grid)
            optimizer.step()
        ...
        if val_acc > opt_val_acc:
            ...
            opt_val_acc = val_acc
            for x, y in model.net.params.items():
                opt_params[x] = y
            #############################################################################
            #                             END OF YOUR CODE                              #
            #############################################################################
