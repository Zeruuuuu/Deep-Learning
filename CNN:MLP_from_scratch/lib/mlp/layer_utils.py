...
from lib.grad_check import eval_numerical_gradient_array
...
class sequential(object):
    ...
    def apply_l1_regularization(self, lam):
        ...
        for layer in self.layers:
            for n, v in layer.grads.items():
                ######## TODO ########
                l1_grad = lam * np.sign(layer.params[n])
                layer.grads[n] = layer.grads[n] + l1_grad
                ######## END  ########
    ...
    def apply_l2_regularization(self, lam):
        ...
        for layer in self.layers:
            for n, v in layer.grads.items():
                ######## TODO ########
                l2_grad = 2 * lam * layer.params[n]
                layer.grads[n] = layer.grads[n] + l2_grad
                ######## END  ########
...
class flatten(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        dfeat = dprev
        dfeat = dfeat.reshape(self.shape)
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
        self.meta = None
...
class fc(object):
    ...
    def backward(self, dprev):
        ...
        #############################################################################
        # dL/dw = dL/dO * dO/dw, and all the same thing for x and b
        self.grads[self.w_name] = (feat.T).dot(dprev)
        self.grads[self.b_name] = (np.ones((1, feat.shape[0]))).dot(dprev)
...
class gelu(object):
    ...
    def backward(self, dprev):
        ...
        # Store the output gradients in the variable dfeat provided above.          #
        #############################################################################
        # dL/dx = dL/dO * dO/dx = dprev * dO/dx
        grid = 0.5 * (1 + np.tanh(np.sqrt(2/np.pi) * (feat + 0.044715 * feat**3))) + 0.5 * feat * \
               (1 - np.tanh(np.sqrt(2/np.pi) * (feat + 0.044715 * feat**3))**2) * np.sqrt(2/np.pi) * (1 + 0.134145 * feat**2)
        dfeat = grid * dprev
        #############################################################################
        #                             END OF YOUR CODE                              #
        #############################################################################
...
class dropout(object):
    ...
    def backward(self, dprev):
        ...
        if self.is_training and self.keep_prob:
            dfeat = self.kept * (1 / self.keep_prob) * dprev
        else:
            dfeat = dprev
        #############################################################################
        #                             END OF YOUR CODE                              #
...
class cross_entropy(object):
    ...
    def forward(self, feat, label):
        ...
        #############################################################################
        idx = range(feat.shape[0])
        logi = logit[idx, label]
        loss = -np.log(logi)
        loss = np.mean(loss)
        #############################################################################
        #                             END OF YOUR CODE                              #
...
def softmax(feat):
    ...
    #############################################################################
    num = np.exp(feat - np.max(feat, axis=-1, keepdims=True))
    dnum = num.sum(axis=-1, keepdims=True)
    scores = num/dnum
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
